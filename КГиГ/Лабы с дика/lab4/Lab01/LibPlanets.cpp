...
{
	double rS = 30, rE = 20, rM = 10, rV = 15, rT = 12, rTM = 6;       // Радиус Солнца, Земли, Луны
	double RoE = 10 * rS, RoM = 5 * rE, RoV = 6*rS, RoT = 15 * rS ;	 // Радиус орбиты Земли и Луны
	double RoTM = 5 * RoT;
	double d = RoE + RoM + rM + RoV + RoT + rTM;		    // Половина диаметра системы
...
//Вычисляет новые координаты планет в СК Солнца через интервал времени dt
{
	...
	double RoTM = (TMoonOrbit.right - TMoonOrbit.left) / 2;// Радиус орбиты Луны    
	double ff1 = (fiM / 180.0) * pi;						// Радианы - угловое положение Луны в СК Земли
	double x1 = RoTM * cos(ff1);							// x - начальная координата Луны в СК Земли
	double y1 = RoTM * sin(ff1);							// y - начальная координата Луны в СК Земли	
	TMCoords(0) = x1;	TMCoords(1) = y1;   TMCoords(2) = 1;
	fiM += wTMoon * dt;
	CMatrix P1 = CreateRotate2D(fiM);					// Матрица поворота против часовой стрелки Луны
	...
	ff = (fiE / 180.0) * pi;								// Радианы - угловое положение Земли в СК Солнца
	x = RoT * cos(ff);									// x - начальная координата Земли в СК Солнца
	y = RoT * sin(ff);
	TCoords(0) = x;
	TCoords(1) = y;
	TCoords(2) = 1;
...
void CSunSystem::Draw(CDC& dc)
{
	...
	int d1 = TMoonOrbit.right;				// Радиус орбиты Луны
